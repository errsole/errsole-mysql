...
class ErrsoleMySQL extends EventEmitter {
  constructor (options = {}) {
    ...
    const { tablePrefix = 'errsole', ...mysqlOptions } = options;
  ...
  async initialize () {
    ...
    this.pool = mysql.createPool(this.mysqlOptions);
  ...
  async createDatabaseIfNotExists () {
    const { host, user, password, database, port, ...rest } = this.mysqlOptions;

    if (!database) {
      throw new Error('Database name must be specified in the options.');
    }

    const connectionConfig = { host, user, password, port, ...rest };
  ...
  async setConfig (key, value) {
    const query = `INSERT INTO \`${this.configTable}\` (\`key\`, \`value\`) VALUES (?, ?) 
  ...
  async flushLogs () {
    ...
    return await new Promise((resolve, reject) => {
      this.pool.getConnection((err, connection) => {
        if (err) return reject(err);
        connection.query(`INSERT INTO ${this.logsTable} (timestamp, hostname, pid, source, level, message, meta, errsole_id) VALUES ?`, [values], () => {
          connection.release();
          if (err) return reject(err);
          resolve({});
        });
