...
class ErrsoleMySQL extends EventEmitter {
  ...
  /**
   * Inserts a notification and retrieves the last notification for the same hostname and hashed_message.
   *
   * @async
   * @function insertNotification
   * @param {string} hostname - The hostname for the notification.
   * @param {number} errsole_id - The Errsole ID.
   * @param {string} hashed_message - The hashed message for the notification.
   * @returns {Promise<{previousNotification: Object, message: string}>} - A Promise that resolves with the previous notification (if any) and a success message.
   * @throws {Error} - Throws an error if the operation fails.
  */

  async insertNotification (hostname, errsoleId, hashedMessage) {
    ...
    try {
      ...
      const previousNotification = await new Promise((resolve, reject) => {
        connection.query(lastNotificationQuery, [hostname, hashedMessage], (err, results) => {
          if (err) return reject(err);
      ...
      return {
        ...
        message: 'New notification inserted successfully'
      };
    } catch (err) {
      await new Promise((resolve, reject) => {
