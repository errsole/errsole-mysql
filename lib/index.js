...
class ErrsoleMySQL extends EventEmitter {
  ...
  async getLogs (filters = {}) {
    ...
    const query = `SELECT * FROM errsole_logs ${whereClause} ORDER BY id ${sortOrder} LIMIT ?`;
  ...
  async searchLogs (searchTerms, filters = {}) {
    ...
    const whereClause = whereClauses.length ? `WHERE ${whereClauses.join(' AND ')}` : '';
    const query = `SELECT * FROM errsole_logs ${whereClause} ORDER BY id DESC LIMIT ?`;
    values.push(filters.limit);
  ...
  async setBufferSizes () {
    ...
    if (currentSize < desiredSize) {
      const query = 'SET SESSION sort_buffer_size = 8388608';
