...
class ErrsoleMySQL extends EventEmitter {
  ...
  /**
   * Retrieves the admin name from the users table.
   *
   * @async
   * @function getAdminName
   * @returns {Promise<string>} - A promise that resolves with the admin name.
   * @throws {Error} - Throws an error if no admin user is found or if the query fails.
   */
  async getAdminName () {
    const query = `SELECT name FROM ${this.usersTable} WHERE role = ?`;
    return new Promise((resolve, reject) => {
      this.pool.query(query, ['admin'], (err, results) => {
        if (err) return reject(err);
        if (!results.length) return reject(new Error('Admin user not found.'));
        console.log(`Admin name found: ${results[0].name}`);
