...
class ErrsoleMySQL extends EventEmitter {
  ...
  async insertNotificationItem (notification = {}) {
    ...
    try {
      ...
      WHERE hostname = ? AND hashed_message = ? AND created_at BETWEEN ? AND ?;
      `;
      const todayNotificationCount = await new Promise((resolve, reject) => {
        connection.query(countTodayNotificationsQuery, [hostname, hashedMessage, startOfDayUTC, endOfDayUTC], (err, result) => {
          if (err) return reject(err);
      ...
      // Return the previous notification and today's count
      return {
