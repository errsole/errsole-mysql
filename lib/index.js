...
class ErrsoleMySQL extends EventEmitter {
  ...
  async insertNotification (hostname, errsoleId, hashedMessage) {
    ...
    try {
      ...
      // Query to fetch the last notification
      const lastNotificationQuery = `
        ...
        LIMIT 1;
      `;
      const lastItem = await new Promise((resolve, reject) => {
      ...
      startOfDayUTC.setUTCHours(0, 0, 0, 0); // Set time to 00:00:00.000 UTC

      const endOfDayUTC = new Date();
      endOfDayUTC.setUTCHours(23, 59, 59, 999); // Set time to 23:59:59.999 UTC

      const dayCountQuery = `
  SELECT COUNT(*) AS count FROM errsole_notifications
  WHERE hostname = ? AND hashed_message = ? 
  AND created_at >= ? AND created_at <= ?
`;

      await new Promise((resolve, reject) => {
        connection.query(dayCountQuery, [hostname, hashedMessage, startOfDayUTC, endOfDayUTC], (err, result) => {
          if (err) return reject(err);
      ...
      // Return the previous notification and success message
      return {
        lastItem,
